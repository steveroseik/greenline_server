...
export class Initial1708086477487 implements MigrationInterface {
    name = 'Initial1708086477487'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`item-price\` (\`id\` int NOT NULL AUTO_INCREMENT, \`price\` decimal(10,2) NOT NULL, \`discount\` decimal(10,2) NULL, \`startDiscount\` timestamp NULL, \`endDiscount\` timestamp NULL, INDEX \`id\` (\`id\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`item-price-list\` (\`itemSku\` varchar(255) NOT NULL, \`itemPriceId\` int NOT NULL, \`currency\` varchar(5) NOT NULL, INDEX \`itemSku\` (\`itemSku\`), PRIMARY KEY (\`itemSku\`, \`itemPriceId\`, \`currency\`)) ENGINE=InnoDB`);
