...
export class Initial1711287442463 implements MigrationInterface {
    name = 'Initial1711287442463'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`user-address\` (\`id\` int NOT NULL AUTO_INCREMENT, \`userId\` varchar(255) NOT NULL, \`countryId\` int NOT NULL, \`cityId\` int NOT NULL, \`address\` varchar(255) NOT NULL, \`createdAt\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`lastModified\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), INDEX \`userId\` (\`userId\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`order\` (\`id\` int NOT NULL AUTO_INCREMENT, \`otherId\` int NULL, \`type\` enum ('delivery', 'exchange', 'refund', 'freeDelivery', 'gift', 'freeOfCharge') NOT NULL DEFAULT 'delivery', \`paymentType\` enum ('cash', 'card', 'free') NOT NULL DEFAULT 'cash', \`status\` enum ('idle', 'shippedFromCourier', 'transferring', 'assignedToCourier', 'outForDelivery', 'delivered', 'partiallyDelivered', 'failedDeliveryAttempt', 'postponed') NOT NULL DEFAULT 'idle', \`userId\` varchar(255) NOT NULL, \`merchantId\` int NOT NULL, \`userAddressId\` int NOT NULL, \`shippingPrice\` decimal(10,2) NOT NULL DEFAULT '0.00', \`weight\` decimal(10,2) NOT NULL DEFAULT '0.00', \`includesVAT\` tinyint(1) NOT NULL, \`canOpen\` tinyint NOT NULL DEFAULT 0, \`fragile\` tinyint NOT NULL DEFAULT 0, \`deliveryPart\` tinyint NOT NULL DEFAULT 0, \`createdAt\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`lastModified\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), INDEX \`userAddressId\` (\`userAddressId\`), INDEX \`merchantId\` (\`merchantId\`), INDEX \`userId\` (\`userId\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`merchant\` (\`id\` int NOT NULL AUTO_INCREMENT, \`name\` varchar(255) NOT NULL, \`includesVAT\` tinyint(1) NOT NULL, \`threshold\` decimal(10,2) NOT NULL DEFAULT '3.00', \`basicShipping\` decimal(10,2) NOT NULL DEFAULT '50.00', \`overShipping\` decimal(10,2) NOT NULL DEFAULT '10.00', \`createdAt\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`lastModified\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`item\` (\`sku\` varchar(255) NOT NULL, \`merchantSku\` varchar(255) NOT NULL, \`merchantId\` int NOT NULL, \`name\` varchar(255) NOT NULL, \`color\` varchar(50) NULL, \`colorCode\` varchar(50) NULL, \`size\` varchar(15) NULL, \`description\` varchar(255) NOT NULL DEFAULT '', \`imageUrl\` varchar(255) NOT NULL, \`weight\` decimal(10,2) NULL, \`createdAt\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`lastModified\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), INDEX \`merchantId\` (\`merchantId\`), PRIMARY KEY (\`sku\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`item-in-box\` (\`id\` int NOT NULL AUTO_INCREMENT, \`inventoryId\` int NOT NULL, \`merchantId\` int NOT NULL, \`itemSku\` varchar(255) NOT NULL, \`boxId\` int NOT NULL, \`count\` int NOT NULL, \`minCount\` int NOT NULL, \`createdAt\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`lastModified\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), INDEX \`boxId\` (\`boxId\`), INDEX \`itemSku\` (\`itemSku\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`inventory-item-history\` (\`id\` int NOT NULL AUTO_INCREMENT, \`itemInBoxId\` int NOT NULL, \`description\` varchar(255) NOT NULL, \`type\` enum ('import', 'newImport', 'orderReturn', 'export', 'orderExport') NOT NULL, \`amount\` int NOT NULL, \`createdAt\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`lastModified\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), INDEX \`itemInBoxId\` (\`itemInBoxId\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`financial-transaction-status\` (\`requestId\` int NOT NULL, \`status\` enum ('pendingSender', 'pendingReceiver', 'processingBySender', 'processingByReceiver', 'rejectedBySender', 'rejectedByReceiver', 'cancelledBySender', 'completed', 'failed') NOT NULL, \`createdAt\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), PRIMARY KEY (\`requestId\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`financial-transaction\` (\`id\` int NOT NULL AUTO_INCREMENT, \`type\` enum ('deposit', 'withdrawal', 'transfer', 'refund', 'adjustment', 'interest', 'donation', 'conversion', 'reward', 'subscription', 'rent', 'payment', 'other') NOT NULL, \`description\` varchar(255) NULL, \`fromAccountId\` varchar(255) NOT NULL, \`toAccountId\` varchar(255) NOT NULL, \`amount\` decimal(10,2) NULL, \`receipt\` varchar(255) NULL, \`status\` enum ('pendingSender', 'pendingReceiver', 'processingBySender', 'processingByReceiver', 'rejectedBySender', 'rejectedByReceiver', 'cancelledBySender', 'completed', 'failed') NOT NULL, \`approvedById\` varchar(255) NULL, \`createdAt\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), INDEX \`approvedById\` (\`approvedById\`), INDEX \`toAccountId\` (\`toAccountId\`), INDEX \`fromAccountId\` (\`fromAccountId\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`DROP TABLE \`financial-transaction-status\``);
        await queryRunner.query(`DROP INDEX \`id\` ON \`item-price\``);
        await queryRunner.query(`DROP TABLE \`item-price\``);
        await queryRunner.query(`DROP INDEX \`itemInBoxId\` ON \`inventory-item-history\``);
        await queryRunner.query(`DROP TABLE \`inventory-item-history\``);
